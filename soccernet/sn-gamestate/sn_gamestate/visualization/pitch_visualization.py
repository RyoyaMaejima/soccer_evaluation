...
class PitchVisualizationEngine(Callback):
    ...
    #追加
    def export_to_json(self, detections, output_path):
        # NaN値を空文字列に変換
        detections = detections.fillna("")

        # jersey_number → jersey に変更した列を作る
        detections = detections.rename(columns={"jersey_number": "jersey"})
        
        # 必要なカラムを抽出
        selected_columns = ["bbox_pitch", "track_id", "jersey", "role", "team"]
        grouped_detections = detections.groupby("image_id")[selected_columns].apply(lambda x: x.to_dict(orient="records"))

        # JSON形式のデータを構築
        json_data = {image_id: records for image_id, records in grouped_detections.items()}
    ...
    def run(self, tracker_state: TrackerState, video_id, detections, image_preds, video_metadata, progress=None):
        ...
        if self.cfg.save_videos:
            ...
            #追加
            filepath = self.save_dir / "datas" / f"{video_name}.json"
            filepath.parent.mkdir(parents=True, exist_ok=True)
            self.export_to_json(detections, filepath)
