...
def feature_column_names(fs: list[FeatureTransfomer], nb_prev_actions: int = 3) -> list[str]:
    ...
    spadlcolumns = [
        ...
        "type_id",
        "type_name",
    ] + [f"location_x_p{i}" for i in range(10)] + [f"location_y_p{i}" for i in range(10)]
...
def gamestates_loc(action_locations : pd.DataFrame, players_per_action : int, nb_prev_actions: int = 3) -> list[pd.DataFrame]:
    if nb_prev_actions < 1:
        raise ValueError("The game state should include at least one preceding action.")
    states = [action_locations]
    for i in range(1, nb_prev_actions):
        def shift_and_fill(group):
            shifted = group.shift(i * players_per_action)
            first_action = group.iloc[:players_per_action].copy()
            repeated = pd.concat([first_action] * i, ignore_index=True)
            shifted.iloc[:(i * players_per_action)] = repeated.values
            return shifted
        
        prev_locations = action_locations.groupby(["game_id", "period_id"], sort=False, as_index=False, group_keys=False).apply(
            shift_and_fill
        )
        prev_locations.index = action_locations.index.copy()
        states.append(prev_locations)
    return states
...
def play_left_to_right_loc(gamestates_loc: list[pd.DataFrame], home_team_id : int) -> list[pd.DataFrame]:
    a0 = gamestates_loc[0]
    away_idx = a0.team_id != home_team_id
    for action_locations in gamestates_loc:
        action_locations.loc[away_idx, "location_x"] = spadlcfg.field_length - action_locations[away_idx]["location_x"].values
        action_locations.loc[away_idx, "location_y"] = spadlcfg.field_width - action_locations[away_idx]["location_y"].values
    return gamestates_loc
...
@simple
def playerlocations(actions: SPADLActions) -> Features:
    loc_cols = [col for col in actions.columns if col.startswith("location_x_p") or col.startswith("location_y_p")]
    return actions[loc_cols]
