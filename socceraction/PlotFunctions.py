...
def actions_with_players(
    ...
    if o_locations is not None:
        ...
        if o_teams is None:
            o_teams = ["Unknown"] * len(o_locations)

        if o_names is None:
            o_names = ["" for _ in range(len(o_locations))]

        o_teams = np.asarray(o_teams)
        o_names = np.asarray(o_names)

        if len(o_teams) != len(o_locations) or len(o_names) != len(o_locations):
            raise ValueError("other_player_teams and other_player_names must match length of other_players")
        
        for (x, y), team_name, name in zip(o_locations, o_teams, o_names):
            color = "blue" if team_name == e_team_name else "red"
            ax.scatter(
                x, y,
                color=color,
                s=figsize * 10,
