...
import math
import pandas as pd
from edit.type_def import team_colors
...
def actions_with_players(
    ...
    ax = field(ax=ax, color="white", figsize=figsize, show=False)
    fig = plt.gcf()
    figsize, _ = fig.get_size_inches()
    arrowsize = math.sqrt(figsize)

    # --- ✅ 他の選手の位置をチームごとに色分けして描画 ---
    if o_locations is not None:
        o_locations = np.asarray(o_locations)
        
        if o_locations.shape[1] != 2:
            raise ValueError("other_players must be of shape (N, 2)")
        
        if o_team_names is None:
            o_team_names = ["Unknown"] * len(o_locations)

        if o_names is None:
            o_names = ["" for _ in range(len(o_locations))]

        o_team_names = np.asarray(o_team_names)
        o_names = np.asarray(o_names)

        if len(o_team_names) != len(o_locations) or len(o_names) != len(o_locations):
            raise ValueError("other_player_teams and other_player_names must match length of other_players")
        
        for (x, y), team_name, name in zip(o_locations, o_team_names, o_names):
